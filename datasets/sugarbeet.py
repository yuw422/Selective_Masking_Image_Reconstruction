...
class SugarBeets(LightningDataModule):
    identifier = "sugarbeets"
    ...
    def __init__(self, batch_size: int, data_list=None, aug=None):
        ...
        def get_samples(subset, data_list):
            dir_sugarbeets = 'sugarbeets/ijrr_sugarbeets_2016_annotations'
            directory = os.path.join(config.DATASET_DIRECTORY, dir_sugarbeets)

            if data_list is None:
                data_list_fn = directory + "/sugarbeets.txt"
                print(f"Loading Sugarbeets, file list {data_list_fn}")
                count = 0
                data_list = []
                ...
                count = len(data_list)
                print(f"Loading Sugarbeets from predetemined data list")
            ...
            return [(directory + SugarBeets.get_rgb_path(x)) for x in data_list]
    ...
    def get_dataset(self, stage: str) -> Optional[Dataset]:
        return SugarBeets._Dataset(self._datasets[stage])
...
class SugarSelectiveMask(SugarBeets):
    identifier = "sugar_selective"
    ...
    def __init__(self, batch_size: int, data_list=None, aug=None):
        ...
        def get_samples(subset, data_list):
            dir_sugar = 'sugarbeets/ijrr_sugarbeets_2016_annotations'
            directory = os.path.join(config.DATASET_DIRECTORY, dir_sugar)
            dir_sugar_selective_mask = 'selective_mask/'
            mask_directory = os.path.join(directory, dir_sugar_selective_mask)
            ...
            print(data_list[0])
            return [(SugarSelectiveMask.get_selective_path(directory +  SugarBeets.get_rgb_path(x)), (directory + SugarBeets.get_rgb_path(x)))
                    for x in data_list]
    ...
    @staticmethod
    def get_selective_path(rgb_path):
        split_address = rgb_path.split("images/rgb")
        fh = 'ijrr_sugarbeets_2016_annotations/selective_mask'.join(
            split_address[0].split('ijrr_sugarbeets_2016_annotations'))
        # print(fh, split_address)
        return fh + split_address[1]

    def get_dataset(self, stage: str) -> Optional[Dataset]:
        return SugarSelectiveMask._Dataset(self._datasets[stage])
...
class SugarSelective(object):
    ...
    def __init__(self, batch_size: int, num_partitions: int):
        ...
        dir_sugar_selective_mask = 'selective_mask/'
        dir_sugar_vis = 'vis/'
        self.selective_dir = os.path.join(config.DATASET_DIRECTORY, dir_sugarbeets, dir_sugar_selective_mask)
        self.vis_dir = os.path.join(config.DATASET_DIRECTORY, dir_sugarbeets, dir_sugar_vis)
        ...
        with open(data_list_fn) as f:
            for line in f:
                ...
                f_sub_dir_name = fn.split('/annotations/')[0]
                Path(os.path.join(self.selective_dir, f_sub_dir_name[1:])).mkdir(exist_ok=True)
                Path(os.path.join(self.vis_dir, f_sub_dir_name[1:])).mkdir(exist_ok=True)
